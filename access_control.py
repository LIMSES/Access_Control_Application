...
def main(_argv):
    ...
    pts = [deque(maxlen = 20) for _ in range(1000)]     # 10 frames, 1000 queue => 최대 1000개의 Bbox 에 대해서 경로를 추적하는데 경로는 30 frame까지 본다.
    # id가 어떻게 부여되는지 몰라서 비효율적으로 크게 1000 잡음
    ...
    while True:
        ...
        # draw bbox on screen
            ...
            if (point_x < int(width/2)) :   # Forward door
                ...
                #cv2.line(frame, (x1, 0), (x1, height), (0,255,0), thickness = 2)              # 들어오는 라인
                #cv2.line(frame, (x2, 0), (x2, height), (0,255,0), thickness = 2)              # 나가는 라인
                ...
                if(int(point_x) < x1 or int(point_x) > x2) :
                    pts[track.track_id].clear()
            elif (point_x > int(width/2)) :                                      # Backward door
